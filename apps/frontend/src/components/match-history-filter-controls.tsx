      ...
      <div className="flex flex-col gap-4 lg:flex-row lg:items-stretch">
        ...
        <div className="flex flex-1 flex-col space-y-2">
          ...
          <Popover open={isClubOpen} onOpenChange={setIsClubOpen}>
            <PopoverTrigger asChild>
              <button type="button" aria-label="Фильтр по клубу" className="flex min-h-12 w-full items-center justify-between rounded-2xl border border-white/10 bg-black/30 px-4 text-left text-sm font-medium text-white outline-none transition-colors hover:border-white/20 focus:border-primary/40">
                <span className="truncate pr-4 text-[13px] leading-tight">
                  {selectedClub ? (() => {
            ...
            <PopoverContent className="min-w-[var(--radix-popover-trigger-width)] p-1.5" align="start">
              <div className="flex flex-col gap-1">
                ...
                setIsClubOpen(false);
              }} className={cn('flex w-full items-center justify-between rounded-xl px-2.5 py-2 text-left text-[13px] font-medium leading-tight transition-colors hover:bg-white/[0.04] hover:text-white', !selectedClub ? 'bg-white/[0.04] text-white' : 'text-slate-300')}>
                  <span className="truncate pr-3">Все клубы</span>
                  {!selectedClub ? <Check className="h-3.5 w-3.5 shrink-0 text-primary" /> : null}
                </button>
                {clubs.map(club => <button key={club.id} type="button" onClick={() => {
                updateFilters({
                  club: club.slug
                });
                setIsClubOpen(false);
              }} className={cn('flex w-full items-center justify-between rounded-xl px-2.5 py-2 text-left text-[13px] font-medium leading-tight transition-colors hover:bg-white/[0.04] hover:text-white', selectedClub === club.slug ? 'bg-white/[0.04] text-white' : 'text-slate-300')}>
                    <span className="truncate pr-3">{`${club.name} — ${club.address}`}</span>
                    {selectedClub === club.slug ? <Check className="h-3.5 w-3.5 srhink-0 text-primary" /> : null}
                  </button>)}
              </div>
            </PopoverContent>
          </Popover>
        ...
        <div className="flex flex-[1.5] flex-col space-y-2">
          ...
          <Popover open={isCalendarOpen} onOpenChange={setIsCalendarOpen}>
            <PopoverTrigger asChild>
              <button type="button" aria-label="Фильтр по периоду" className={cn('flex min-h-12 w-full items-center justify-between rounded-2xl border border-white/10 bg-black/30 px-4 text-left text-sm font-semibold outline-none transition-colors hover:border-white/20 focus:border-primary/40', !dateRange && 'text-slate-400')}>
            ...
            <PopoverContent className="min-w-0 p-3" align="start">
              <div className="rounded-[28px] p-4">
                <RangeCalendar aria-label="Период матчей" firstDayOfWeek="mon" value={calendarRange} onChange={range => {
                updateFilters({
                  range
                });
                setIsCalendarOpen(false);
              }} visibleMonths={1} className="w-full" classNames={{
                base: 'w-full gap-3',
                headerWrapper: 'mb-6 rounded-2xl px-1 py-2',
                header: 'gap-3',
                title: 'flex-1 text-center font-mono text-[11px] font-black uppercase tracking-[0.28em] text-white',
                prevButton: 'inline-flex h-6 w-9 items-center justify-center rounded-xl border-0 bg-transparent text-slate-300 shadow-none transition-colors hover:bg-transparent hover:text-white',
                nextButton: 'inline-flex h-6 w-9 items-center justify-center rounded-xl border-0 bg-transparent text-slate-300 shadow-none transition-colors hover:bg-transparent hover:text-white',
                content: 'w-full',
                gridWrapper: 'w-full',
                grid: 'w-full border-collapse',
                gridHeaderRow: 'mb-4 grid grid-cols-7',
                gridHeaderCell: 'h-9 w-10 text-center font-mono text-[10px] font-bold uppercase tracking-[0.24em] text-slate-500',
                gridBodyRow: 'grid grid-cols-7',
                cell: 'flex h-10 w-10 items-center justify-center p-0',
                cellButton: 'flex h-10 w-10 items-center justify-center rounded-xl text-center text-sm font-semibold text-slate-200 transition data-[disabled=true]:cursor-not-allowed data-[disabled=true]:text-slate-700 data-[hover=true]:bg-white/[0.06] data-[hover=true]:text-white data-[outside-month=true]:text-slate-700 data-[pressed=true]:scale-[0.98] data-[selected=true]:bg-primary/20 data-[selected=true]:font-black data-[selected=true]:text-primary data-[selection-start=true]:bg-primary data-[selection-start=true]:text-black data-[selection-end=true]:bg-primary data-[selection-end=true]:text-black data-[today=true]:ring-1 data-[today=true]:ring-primary/50'
              }} />
              </div>
            </PopoverContent>
          </Popover>
        ...
        <div className="flex flex-none flex-col space-y-3">
          <span className="select-none font-mono text-base uppercase tracking-[0.24em] text-transparent" aria-hidden="true">
            Сброс
          </span>
